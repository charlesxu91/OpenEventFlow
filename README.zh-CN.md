...
- Collector：schema 校验、raw/enriched topic、bad-event 路由
...
- tracking-plan 代码生成
- Collector schema 校验和 bad-event 路由
...
OpenEventFlow 当前处于开源首版阶段。核心 SDK、Collector、Warehouse、代码生成、部署模板、本地 e2e 验证链路已经实现。下一阶段重点是持久化队列、生产级鉴权、可观测性看板，以及更多框架级自动埋点适配器。
